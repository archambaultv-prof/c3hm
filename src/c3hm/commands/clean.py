...
def remove_unwanted_dirs(root_path: Path | str | None = None,
                         paths_to_delete: list[str] | None = None,
                         exclude_dir: list[str] | None = None,
                         verbose: bool = False,
                         dryrun: bool = False,
                         only_student_folder: bool = True) -> None:
    """
    Parcourt chaque dossier étudiant sous root_path et supprime
    tout fichier ou dossier dont le nom apparaît dans l'ensemble
    PATHS_TO_DELETE. Saute les dossiers qui correspondent à des motifs
    dans exclude_dir.
    ...
    only_student_folder: Si True, ne traite que les dossiers étudiants.
    Si False, traite tous les fichiers et dossiers dans root_path.
    """
        ...
        for current_root, dirs, files in os.walk(root_path, topdown=True):
            ...
            for d in list(dirs):
                ...
                if is_path_to_delete(d_path, paths_to_delete):
                    ...
                    if not dryrun:
                        shutil.rmtree(d_path, onexc=remove_readonly)
